...
class ContinuousMountainCar(environment.Environment):
    ...
    def step(
        ...
        force = jnp.clip(
            action, self.env_params["min_action"], self.env_params["max_action"]
        )
        velocity = (
            state["velocity"]
            + force * self.env_params["power"]
            - jnp.cos(3 * state["position"]) * self.env_params["gravity"]
        )
        velocity = jnp.clip(
            velocity, -self.env_params["max_speed"], self.env_params["max_speed"]
        )
        position = state["position"] + velocity
        position = jnp.clip(
            position, self.env_params["min_position"], self.env_params["max_position"]
        )
        velocity = velocity * (
            1 - (position >= self.env_params["goal_position"]) * (velocity < 0)
        )

        reward = -0.1 * action ** 2 + 100 * (
            (position >= self.env_params["goal_position"])
    ...
    def action_space(self):
        ...
        return spaces.Box(
            low=self.env_params["min_action"],
    ...
    def observation_space(self):
        ...
        low = jnp.array(
            [self.env_params["min_position"], -self.env_params["max_speed"]],
            dtype=jnp.float32,
        )
        high = jnp.array(
            [self.env_params["max_position"], self.env_params["max_speed"]],
            dtype=jnp.float32,
        )
