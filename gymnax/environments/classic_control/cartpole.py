...
class CartPole(environment.Environment):
    ...
    def step(
        ...
        force = self.env_params["force_mag"] * action - self.env_params["force_mag"] * (
            1 - action
        )
        ...
        xacc = (
            temp
            - self.env_params["polemass_length"]
            * thetaacc
            * costheta
            / self.env_params["total_mass"]
        )
    ...
    def is_terminal(self, state: dict) -> bool:
        ...
        done2 = jnp.logical_or(
            state["theta"] < -self.env_params["theta_threshold_radians"],
            state["theta"] > self.env_params["theta_threshold_radians"],
        )
