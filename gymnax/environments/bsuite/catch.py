...
class Catch(environment.Environment):
    ...
    def __init__(self):
        ...
        self.env_params = FrozenDict(
            {"max_steps_in_episode": 100000000, "rows": 10, "columns": 5}
        )
    ...
    def step(
        ...
        ball_x, ball_y, paddle_x, paddle_y = sample_init_state(key, self.env_params)
        prev_done = state["prev_done"]

        # Move the paddle + drop the ball.
        dx = action - 1  # [-1, 0, 1] = Left, no-op, right.
        paddle_x = (
            jnp.clip(state["paddle_x"] + dx, 0, self.env_params["columns"] - 1)
            * (1 - prev_done)
            + paddle_x * prev_done
        )
    ...
    def observation_space(self):
        ...
        return spaces.Box(
            0, 1, (self.env_params["rows"], self.env_params["columns"]), dtype=jnp.int_
        )
