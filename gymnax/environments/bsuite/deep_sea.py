...
class DeepSea(environment.Environment):
    ...
    def step(
        ...
        right_rand_cond = jnp.logical_or(
            rand_trans_cond, self.env_params["deterministic"]
        )
        right_cond = jnp.logical_and(action_right, right_rand_cond)

        reward, denoised_return = step_reward(
            state, action_right, right_cond, rand_reward, self.env_params
        )
        column, row, bad_episode = step_transition(
            state, action_right, right_cond, self.env_params
        )
        state = {
            "row": row,
            "column": column,
    ...
    def reset(self, key: PRNGKey) -> Tuple[Array, dict]:
        ...
        ) ** (self.env_params["size"] - 1) + self.env_params["deterministic"] * 1.0
        optimal_return = optimal_no_cost - self.env_params["unscaled_move_cost"]

        a_map_rand = jax.random.bernoulli(
            key, 0.5, (self.env_params["size"], self.env_params["size"])
        )
        a_map_determ = jnp.ones([self.env_params["size"], self.env_params["size"]])

        new_a_map_cond = jnp.logical_and(
            1 - self.env_params["deterministic"], self.env_params["sample_action_map"]
        )
        old_a_map_cond = jnp.logical_and(
            1 - self.env_params["deterministic"],
            1 - self.env_params["sample_action_map"],
        )
        action_mapping = (
            self.env_params["deterministic"] * a_map_determ
    ...
    def get_obs(self, state: dict) -> Array:
        ...
        obs_end = jnp.zeros(
            shape=(self.env_params["size"], self.env_params["size"]), dtype=jnp.float32
        )
    ...
    def observation_space(self):
        ...
        return spaces.Box(
            0, 1, (self.env_params["size"], self.env_params["size"]), jnp.float32
        )
        ...
        return spaces.Dict(
            {
                ...
                "action_mapping": spaces.Box(
                    0,
                    1,
                    (self.env_params["size"], self.env_params["size"]),
                    dtype=jnp.int_,
                ),
...
def step_reward(state, action_right, right_cond, rand_reward, params):
    ...
    reward = 0.0
    # Reward calculation.
    rew_cond = jnp.logical_and(state["column"] == params["size"] - 1, action_right)
    reward += rew_cond
    denoised_return = state["denoised_return"] + rew_cond

    # Noisy rewards on the 'end' of chain.
    col_at_edge = jnp.logical_or(
        state["column"] == 0, state["column"] == params["size"] - 1
    )
    chain_end = jnp.logical_and(state["row"] == params["size"] - 1, col_at_edge)
...
def step_transition(state, action_right, right_cond, params):
    ...
    column = (1 - right_cond) * state["column"] + right_cond * jnp.clip(
        state["column"] + 1, 0, params["size"] - 1
    )
