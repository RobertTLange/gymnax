...
class UmbrellaChain(environment.Environment):
    ...
    def __init__(self, chain_length: int = 10, n_distractor: int = 0):
        super().__init__()
    ...
    def step(
        ...
        return (
            lax.stop_gradient(self.get_obs(state, key_distractor)),
    ...
    def get_obs(self, state: dict, key: PRNGKey) -> Array:
        ...
        obs = jnp.zeros(
            shape=(1, 3 + self.env_params["n_distractor"]), dtype=jnp.float32
        )
        ...
        obs = jax.ops.index_update(
            obs,
            jax.ops.index[0, 2],
            1 - state["time"] / self.env_params["chain_length"],
        )
        obs = jax.ops.index_update(
            obs,
            jax.ops.index[0, 3:],
            jax.random.bernoulli(key, p=0.5, shape=(self.env_params["n_distractor"],)),
        )
        return obs
