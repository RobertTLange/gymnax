...
class MemoryChain(environment.Environment):
    ...
    def __init__(self, memory_length: int = 5, num_bits: int = 1):
        super().__init__()
        # Default environment parameters
        self.env_params = FrozenDict(
            {"memory_length": 5, "num_bits": 1, "max_steps_in_episode": 1000}
        )
    ...
    def reset(self, key: PRNGKey) -> Tuple[Array, dict]:
        ...
        context = jax.random.bernoulli(
            key_context, p=0.5, shape=(self.env_params["num_bits"],)
        )
        query = jax.random.randint(
            key_query, minval=0, maxval=self.env_params["num_bits"], shape=()
        )
        state = {
            "context": context,
            "query": query,
    ...
    def get_obs(self, state: dict) -> Array:
        ...
        obs = jnp.zeros(shape=(1, self.env_params["num_bits"] + 2), dtype=jnp.float32)
        ...
        query_val = lax.select(
            state["time"] == self.env_params["memory_length"] - 1, state["query"], 0
        )
