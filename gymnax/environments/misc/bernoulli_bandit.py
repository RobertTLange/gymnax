...
params_bandit = FrozenDict({"sample_probs": jnp.array([0.1, 0.9]), "max_steps": 100})
...
def step(rng_input, params, state, action):
    ...
    obs = get_obs(reward, action, timestep, params)
    state = jax.ops.index_update(state, 2, timestep)
...
def reset(rng_input, params):
    ...
    p1 = jax.random.choice(rng_input, params["sample_probs"], shape=(1,)).squeeze()
