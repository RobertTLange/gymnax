...
class Discrete(jittable.Jittable):
    ...
    def sample(self, rng: PRNGKey) -> Array:
        ...
        return jax.random.randint(rng, shape=self.shape, minval=0,
                                  maxval=self.num_categories-1).astype(self.dtype)
...
class Box(jittable.Jittable):
    ...
    def sample(self, rng: PRNGKey) -> Array:
        ...
        return jax.random.uniform(rng, shape=self.shape,
                                  minval=self.low,
...
class Dict(jittable.Jittable):
    ...
    def sample(self, rng: PRNGKey) -> dict:
        ...
        return OrderedDict([(k, self.spaces[k].sample(key_split[i])) for i,
                             k in enumerate(self.spaces)])
...
class Tuple(jittable.Jittable):
    ...
    def sample(self, rng: PRNGKey) -> Tuple[Array]:
        """ Sample random action from all subspaces. """
        return tuple([self.spaces[k].sample(key_split[i]) for i,
                      k in enumerate(self.spaces)])
