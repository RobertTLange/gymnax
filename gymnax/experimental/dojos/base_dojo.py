...
class BaseDojo(object):
    ...
    def actor_learner_step(self, carry_input, tmp):
        ...
        action, actor_state = self.action_selection(
            key_act, obs, agent_params, actor_state
        )
        ...
        agent_params, learner_state = self.update_learner(
            key_learn, agent_params, learner_state
        )
        ...
        next_state = jax.tree_multimap(
            lambda a, b: done * a + (1 - done) * b, state_reset, next_state
        )
    ...
    def lax_rollout(
        self, key_input, num_steps, agent_params, actor_state, learner_state
    ):
        ...
        scan_out1, scan_out2 = lax.scan(
            ...
            [key_input, obs, state, agent_params, actor_state, learner_state],
            [jnp.zeros(num_steps)],
        )
    ...
    def vmap_rollout(
        ...
        rollout_map = vmap(
            self.lax_rollout, in_axes=(0, None, None, None, None), out_axes=0
        )
        traces, rewards = rollout_map(
            key_input, num_steps, agent_params, actor_state, learner_state
        )
    ...
    def batch_rollout(self, key_rollout, num_steps, agent_params=None):
        ...
        try:
            # Different cases: agent_params explicitly supplied/when not
            if agent_params is None:
                traces, rewards = self.vmap_rollout(
                    key_rollout,
                    num_steps,
                    self.agent_params,
                    self.actor_state,
