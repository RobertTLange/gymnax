...
def push_buffer(buffer, step_experience):
    ...
    buffer["state"] = jax.ops.index_update(
        buffer["state"], jax.ops.index[buffer["pointer"], :], step_experience["state"]
    )
    buffer["next_state"] = jax.ops.index_update(
        buffer["next_state"],
        jax.ops.index[buffer["pointer"], :],
        step_experience["next_state"],
    )
    buffer["obs"] = jax.ops.index_update(
        buffer["obs"], jax.ops.index[buffer["pointer"], :], step_experience["obs"]
    )
    buffer["next_obs"] = jax.ops.index_update(
        buffer["next_obs"],
        jax.ops.index[buffer["pointer"], :],
        step_experience["next_obs"],
    )
    buffer["action"] = jax.ops.index_update(
        buffer["action"], jax.ops.index[buffer["pointer"], :], step_experience["action"]
    )
    buffer["reward"] = jax.ops.index_update(
        buffer["reward"], jax.ops.index[buffer["pointer"]], step_experience["reward"]
    )
    buffer["done"] = jax.ops.index_update(
        buffer["done"], jax.ops.index[buffer["pointer"]], step_experience["done"]
    )

    # Update the buffer pointer, reset once capacity is full - overwrite
    buffer["pointer"] += 1
    buffer["pointer"] = buffer["pointer"] % buffer["capacity"]
    buffer["total_transitions"] = jnp.minimum(
        buffer["total_transitions"] + 1, buffer["capacity"]
    )
    return buffer
...
def sample_buffer(key, buffer, batch_size):
    ...
    sample_idx = jax.random.randint(
        key, shape=(batch_size,), minval=0, maxval=buffer["total_transitions"]
    )
