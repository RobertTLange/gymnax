...
def test_step():
    ...
    for ep in range(num_episodes):
        ...
        for s in range(num_steps):
            ...
            reward_gym = step_agent_numpy(env_gym, action_gym)
            state_jax_post_agent, r, w = step_agent(action, state, env_jax.params)
            assert_correct_state(env_gym, env_name_jax, state_jax_post_agent,
                                 tolerance)

            # Perform step transition for cars & assert correct state dict
            state_gym_post_agent = np_state_to_jax(env_gym, env_name_jax)
            step_cars_numpy(env_gym)
            state_jax_post_cars = step_cars(state_gym_post_agent)
            assert_correct_state(env_gym, env_name_jax, state_jax_post_cars,
                                 tolerance)
